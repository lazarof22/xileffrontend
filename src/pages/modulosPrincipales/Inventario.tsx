...
export default function InventoryPage() {
    ...
    const fetchKardex = async () => {
        ...
        try {
            let productosMap = new Map(rows.map(r => [r.id, r.producto]));

            if (productosMap.size === 0) {
                const prodResponse = await fetch(`${API_URL}/producto`);
                if (prodResponse.ok) {
                    const prodResult = await prodResponse.json();
                    const prodData = Array.isArray(prodResult) ? prodResult : prodResult.data || [];
                    productosMap = new Map(prodData.map((p: any) => [p._id, p.nombre_producto]));
                }
            }
            ...
            const mappedData = data.map((k: any) => ({
                ...
                producto: productosMap.get(k.productoId?.toString()) || 'Producto desconocido',
                tipo: k.tipo,
                cantidad: k.cantidad,
                motivo: k.motivo,
                stockFinal: '-',
